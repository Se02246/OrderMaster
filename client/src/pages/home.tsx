...
import Apartmeimport { useState, useRef } from "react";
import { useQuery, useMutation } from "@tanstack/react-query";
import ApartmentCard from "@/components/ui/data-display/ApartmentCard";
...
export default function Home() {
  ...
  const handleColorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (delta !== 0) {
      switch (cmax) {
        case r_: h = 60 * (((g_ - b_) / delta) % 6); break;
        case g_: h = 60 * (((b_ - r_) / delta) + 2); break;
        case b_: h = 60 * (((r_ - g_) / delta) + 4); break;
      }
    }
    if (h < 0) h += 360;

    const primaryHue = `${h.toFixed(0)} 84% 43%`;
    document.documentElement.style.setProperty('--primary', primaryHue);
    localStorage.setItem("themeColor", primaryHue);
  };
...
}ntModal from "@/components/ui/modals/ApartmentModal";
import ConfirmDeleteModal from "@/components/ui/modals/ConfirmDeleteModal";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { apiRequest, queryClient } from "@/lib/queryClient";
